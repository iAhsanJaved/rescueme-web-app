    ...
    $('#input-organization-select').on('change', function() {
        ...
        if(languagesID !="" && languages!="" && devices !="" && devicesID!="" ) {
            ...
            for(var i=0; i<languages.length-1; i++) {
                ...
                rawHTML += '<input type="text" name="question[]" class="form-control form-control-alternative" placeholder="Question" required autofocus>'
                rawHTML += '</div>'
                $("#dynamic-inputs").append(rawHTML)
                rawHTML = ""
            }
...
$(function () {
    $('input[name="expire_at"]').datetimepicker();
});
